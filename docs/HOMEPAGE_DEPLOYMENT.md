...
Wykrywanie formularzy jest włączone w panelu (potwierdzone zrzutem właściciela 9 września 2026). Odbiorcę powiadomień konfiguruje się w Netlify: Project configuration → Notifications → Emails and webhooks → Form submission notifications. Dla `New form submission` należy wskazać prywatny adres powiadomień właściciela, z wyborem `kontakt` lub `Any form`. Adres nie jest publikowany w kodzie strony ani dokumentacji. Dostarczenie wiadomości wymaga osobnego potwierdzenia testowym zgłoszeniem i sprawdzeniem skrzynki odbiorczej.

Po wdrożeniu sprawdź, czy Netlify usunął atrybut `data-netlify` z publicznego HTML i zachował pole `form-name`. Następnie wyślij testowe zgłoszenie, sprawdź Forms → kontakt oraz skonfigurowaną skrzynkę odbiorczą (także Spam). Zrzut lub status HTTP sam w sobie nie potwierdza dostarczenia e-maila. Komunikat o danych opisuje faktyczny przepływ przez Netlify i skrzynkę odbiorczą; pełne dane formalne administratora nadal wymagają uzupełnienia w odrębnym zakresie.
...
## Sekcja usługi Shoper — 12 września 2026

Strona główna zawiera dedykowaną sekcję `#shoper` pomiędzy przeglądem usług a procesem współpracy. Prowadzą do niej linki z nawigacji głównej, mobilnej, wiersza usług i stopki. Sekcja opisuje budowę sklepów od podstaw oraz przebudowę istniejących szablonów, własne rozwiązania HTML i CSS zgodne ze strukturą Storefront, zakres elementów sklepu, zastosowania B2B i B2C oraz sytuacje, w których usługa jest właściwa.

Układ wykorzystuje oryginalny lokalny znak Shoper i istniejące zasoby strony. Na szerokich ekranach zakres oraz scenariusze są prezentowane w dwóch kolumnach; poniżej 720 px przechodzą do jednej kolumny z zachowaniem pełnych odstępów i naturalnego łamania tekstu. CTA wybiera w formularzu istniejącą opcję `Strona, sklep lub Shoper`, więc integracja Netlify Forms i schemat przesyłanych pól pozostają bez zmian.

Treść nie deklaruje gotowych rezultatów sprzedażowych ani nie zastępuje planowanej podstrony `/sklepy-internetowe/sklepy-shoper`. Sekcja strony głównej przedstawia zakres kompetencji i kieruje do rozmowy, a przyszła podstrona zachowuje rolę szczegółowego celu dla intencji platformowej.
